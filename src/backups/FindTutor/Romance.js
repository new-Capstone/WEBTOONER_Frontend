...
//Romance에 해당하는 튜터아이디와 튜터 이미지 가져오기
...
const Romance = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        setRomancePortfolioList(response.data);
      })
      .catch((error) => console.log(error));
  }, []);

  console.log(romancePortfolioList);
  ...
  const records = romancePortfolioList.slice(firstIndex, lastIndex);

  const npage = Math.ceil(romancePortfolioList.length / recordsPerPage);
